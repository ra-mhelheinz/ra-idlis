...
<div class="content p-4">
    ...
    <div class="card">
        <div class="card-header bg-white font-weight-bold">
           Manage Groups <span class="MA07_add"><a href="#" title="Add New Group" data-toggle="modal" data-target="#myModal"><button class="btn-primarys"><i class="fa fa-plus-circle"></i>&nbsp;Add new</button></a></span>
                ...
                @foreach ($GR as $GRs)
                  <tr>
                    ...
                    <td>
                      <center>
                        <span class="">
                          <button type="button" class="btn-defaults" onclick="showData('{{$GRs->grp_id}}', '{{$GRs->grp_desc}}');" data-toggle="modal" data-target="#GodModal"><i class="fa fa-fw fa-edit"></i></button>
                        </span>
                        <span class="">
                          <button type="button" class="btn-defaults" onclick="showDelete('{{$GRs->grp_id}}', '{{$GRs->grp_desc}}');" data-toggle="modal" data-target="#DelGodModal"><i class="fa fa-fw fa-trash"></i></button>
                        </span>
                      </center>
                    </td>
                  </tr>
      ...
      $(document).ready(function() {
       ...
       $('#NewRight').on('submit',function(event){
            ...
            $.ajax({
                ...
                success: function(data) {
                  if (data == 'DONE') {
                      ...
                      window.location.href = "{{ asset('employee/dashboard/grouprights/mng_grps') }}";
                  } else if (data == 'ERROR') {
          ...
          event.preventDefault();
            ...
            form.parsley().validate();
             if (form.parsley().isValid()) {
               ...
               var y = $('#edit_desc').val();
               $.ajax({
                  url: "{{ asset('/employee/dashboard/grouprights/mng_grps/update') }}",
                  method: 'POST',
        ...
        function deleteNow(){
          ...
          $.ajax({
            url : "{{ asset('/employee/dashboard/grouprights/mng_grps/delete') }}",
            method: 'POST',
